...
public abstract class NiFiSystemIT {
    ...
    @Rule
    public Timeout defaultTimeout = new Timeout(5, TimeUnit.MINUTES);  // OffloadIT (5 iterations) needs more than PT3M

    @Rule(order = Integer.MIN_VALUE)
    public TestWatcher quarantineRule = new TestWatcher() {
        @Override
        protected void failed(final Throwable t, final Description description) {
            final String folderName = testCounter.get() + "-" + description.getMethodName();
            try {
                final File dir = quarantineTroubleshootingInfo(folderName, t);
                logger.info("Test failure for <{}>. Successfully wrote troubleshooting info to {}", folderName, dir.getAbsolutePath());
            } catch (final Exception e) {
                logger.error("Failed to quarantine troubleshooting info for test " + folderName, e);
            }
        }
    };
    ...
    public void setup() throws IOException {
        ...
        if (nifiRef.get() == null) {
            ...
            instance.start();
            logger.info("NiFi instance(s) started; PID(s)={}", instance.getProperty("pid"));
    ...
    public void teardown() throws Exception {
        try {
            ...
            logger.error("Failure during test case teardown (#247)", e);
        } finally {
            if (nifiClient != null) {
                nifiClient.close();
            }
